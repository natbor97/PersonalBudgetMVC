...
class balance extends \Core\Model
{
    ...
    public static function getExpenses($date, $id) 
    {
        ...
        $stmt = $db->prepare( 'SELECT exp.id, exp.amount, exp.date_of_expense, exp.expense_category_assigned_to_user_id, exp.payment_method_assigned_to_user_id, exp.expense_comment, cat.name as expense_name, pay.name as payment_name FROM expenses AS exp, expenses_category_assigned_to_users AS cat, payment_methods_assigned_to_users AS pay WHERE exp.date_of_expense BETWEEN :start_date AND :end_date AND exp.user_id = :user_id AND pay.user_id = :user_id AND  exp.expense_category_assigned_to_user_id = cat.id AND exp.payment_method_assigned_to_user_id = pay.id ORDER BY exp.date_of_expense ASC' );
    ...
    public static function getExpensesSum($date, $id ) 
    {
        ...
        $stmt = $db->prepare( 'SELECT ROUND(SUM(exp.amount), 2) as expenseSumArray, cat.name as expense_name FROM expenses as exp, expenses_category_assigned_to_users as cat WHERE exp.user_id = :user_id AND exp.user_id = cat.user_id AND cat.id = exp.expense_category_assigned_to_user_id AND exp.date_of_expense BETWEEN :start_date AND :end_date GROUP BY cat.name'); 
